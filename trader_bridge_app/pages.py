...
class ResumeTradingSession(WaitPage):
    ...
    @staticmethod
    def is_displayed(player: Player):
        # On the first round of a market the trading session was just
        # created by SyncTradingSession and is already running; calling
        # /resume on it would 404 because it was never paused.
        if _is_first_round_of_market(player.round_number):
            return False
        return not _group_init_error(player.group)
